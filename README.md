...
- Barbora's app-link files contain at least one retired route that launches the app and then returns a 404. Links therefore always preserve the crawler's current website path. Android tries that live URL through an explicit `intent://` link and falls back to the same HTTPS page; iOS uses the ordinary HTTPS/Universal Link behavior. On iOS the preference may have to be granted once — long-press a category link in Notes and choose **Open in Barbora**.
