...
test('rapid closes are serialized and an unpushed layer needs no cleanup', () => {
  const { nav, state } = browser()
  nav.add('detail', () => {})
  nav.add('menu', () => {})
  nav.drop('menu')
  nav.add('confirmation', () => {})
  nav.drop('confirmation')
  nav.drop('detail')
  assert.equal(state.pops.length, 1, 'only one back navigation may be in flight')
  nav.onPop()
  assert.equal(state.pops.length, 2)
  nav.onPop()
  assert.equal(state.pops.length, 2, 'an unpushed layer needs no history cleanup')
  assert.equal(nav.depth, 0)
})

test('a layer opened while history is cleaning is pushed when that navigation settles', () => {
  const { nav, state } = browser()
  nav.add('detail', () => {})
  nav.add('menu', () => {})
  nav.drop('menu')
  ...
  assert.equal(state.entries, 1)
  nav.onPop()
  assert.equal(state.entries, 2, 'the waiting layer now owns a history entry')
})
